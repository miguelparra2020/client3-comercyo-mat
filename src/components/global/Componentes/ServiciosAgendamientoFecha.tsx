...
const ServiciosAgendamientoFecha = () => {
  ...
  const fetchCalendarEvents = async (date: Date, calendarId: string) => {
    ...
    const useCurrentDateAsWorkaround = false
      ...
      const url = `https://backend-MAT Pro Studio.vercel.app/api/calendar/events?calendarId=${encodeURIComponent(calendarId)}&periodStart=${periodStart}&periodEnd=${periodEnd}`
